%% Poincare section
% Param to have a cycle a from 1.9 to 2.1
a = 2.1;
b = 1.7;
...
% sezione di Poincare'
figure;
start = floor(9*length(T)/10); % per levare as much as possible the transitorio
plot3(X(start:end,1),X(start:end,2),X(start:end,3),'b');
hold on;
h1=surface(x,y,z,'facecolor',[1.0000 0.5020 0],'edgecolor','none','facealpha',0.75);
...
for a=A
    ...
    [T, X] = ode45(@fnReaz, [136000 start], x0, opt, a, b);
...
opt = odeset('RelTol', 1.0e-5, ...
             'Refine', 32);
[T2, X] = ode45(@lorenz, [0 10], X0, opt, par);
