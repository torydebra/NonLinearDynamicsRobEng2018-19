...
zlabel('z');
figure
plot(Xe(starte:end,1),Xe(starte:end,2),'ro');
xlabel('x');
ylabel('y');
...
% Crea diagramma di biforcazione al variare di a
...
opt = odeset('RelTol', 1.0e-5);
% Vettore dei valori di a per i quali calcolo il diagramma di biforcazione
...
for a=A
    ...
    % Ricavo numericamente i massimi di z a partire dai risultati
    % dell'integrazione numerica
    [p, t] = dsfindpeaks(X(:, 3));
    
    % Visualizzo i massimi di z, ossia le intersezioni della traiettoria
