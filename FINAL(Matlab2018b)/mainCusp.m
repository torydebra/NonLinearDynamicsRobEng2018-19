...
disp(' ');
disp(['COMMENTO: Sono state trovate due biforcazioni fold per p1 = ',num2str(xE(4,sE(2).index)),'e p1 = ',num2str(xE(4,sE(3).index))]);
...
disp(' ');
disp(['COMMENTO: E'' stata trovata una biforcazione cusp per (p1,p2) = (',num2str(xLP(4,sLP(2).index)),',',num2str(xLP(5,sLP(2).index)),')']);
...
plot3(xLP(4,:),xLP(5,:),repmat(minX,sz),'g','LineWidth',2);
plot3(repmat(maxP1,sz),xLP(5,:),xLP(1,:),'g','LineWidth',2);
plot3(xLP(4,:),repmat(maxP2,sz),xLP(1,:),'g','LineWidth',2);
